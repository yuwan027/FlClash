...
// implementation of CCM, like crypto/aes. NewCCM will check for this interface
// and return the specific CCM if found.
type ccmAble interface {
	NewCCM(nonceSize, tagSize int) (CCM, error)
}
...
func NewCCM(b cipher.Block) (CCM, error) {
	return newCCMWithNonceAndTagSize(b, ccmStandardNonceSize, ccmTagSize)
}
...
func NewCCMWithNonceSize(b cipher.Block, size int) (CCM, error) {
	return newCCMWithNonceAndTagSize(b, size, ccmTagSize)
}
...
func NewCCMWithTagSize(b cipher.Block, tagSize int) (CCM, error) {
	return newCCMWithNonceAndTagSize(b, ccmStandardNonceSize, tagSize)
}

// NewCCMWithSize returns the given 128-bit block cipher wrapped in CCM.
// The tagsize must be an even integer between 4 and 16 inclusive
// and is used as CCM's `M` parameter.
func NewCCMWithSize(b cipher.Block, size, tagSize int) (CCM, error) {
	return newCCMWithNonceAndTagSize(b, size, tagSize)
}

func newCCMWithNonceAndTagSize(cipher cipher.Block, nonceSize, tagSize int) (CCM, error) {
	if tagSize < ccmMinimumTagSize || tagSize > ccmTagSize || tagSize&1 != 0 {
	...
	c := &ccm{cipher: cipher, M: uint8(tagSize), L: uint8(lensize)}
	return c, nil
}
